...
const TOKEN_LIFETIME_SECONDS: i64 = 1800; // 30 minutes
...
impl AuthService {
    ...
    pub fn generate_jwt_token(&self, user: &User) -> AppResult<(String, DateTime<Utc>)> {
        let now = Utc::now();
        let expires = now + Duration::seconds(TOKEN_LIFETIME_SECONDS);
    ...
    pub async fn extend_jwt_token(&self, token: &str) -> AppResult<(String, DateTime<Utc>)> {
        ...
        let new_expires = base + Duration::seconds(TOKEN_LIFETIME_SECONDS);
